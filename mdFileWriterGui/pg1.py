...
class ElementListGui():
    def __init__(self,root,element_list):
        ...
        list_frame = tk.Frame(main_frame, width=270, bg=vu.COLOR_BACKGOUND)#, highlightbackground='red', highlightthickness=1)
    ...
    def create_element_list(self,frame):
        ...
        for i in range(len(self.element_list)):
            ...
            label1 = tk.Label(iframe, text=self.element_list[i][0], height=0,
                                bg=vu.COLOR_BACKGOUND, fg=vu.TEXT_COLOR_2,
                                font=(vu.MAIN_FONT, 12))
            label1.place(x=10, y=4)

            label2 = tk.Label(iframe, text=self.element_list[i][1], height=0,
    ...
    def del_button(self,content_frame):
        if self.element_list:
            self.element_list.pop(-1)
        print("del")
        self.create_element_list(content_frame)
        
    def add_button(self,content_frame):
        self.element_list.append(["new","text text"])
        print("add")
        self.create_element_list(content_frame)